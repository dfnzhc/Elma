...
template<typename T> inline int get_width(const Mipmap<T>& mipmap)
{
    assert(mipmap.images.size() > 0);
    return mipmap.images[0].width;
}

template<typename T> inline int get_height(const Mipmap<T>& mipmap)
{
    assert(mipmap.images.size() > 0);
    return mipmap.images[0].height;
}

template<typename T> inline Mipmap<T> make_mipmap(const Image<T>& img)
{
    Mipmap<T> mipmap;
