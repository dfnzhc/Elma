...
template<typename T> inline const Mipmap<T>& get_img(const ImageTexture<T>& t, const TexturePool& pool)
{
    return Mipmap<T>{};
}

template<> inline const Mipmap<Real>& get_img(const ImageTexture<Real>& t, const TexturePool& pool)
{
    return get_img1(pool, t.texture_id);
}

template<> inline const Mipmap<Vector3>& get_img(const ImageTexture<Vector3>& t, const TexturePool& pool)
{
...
template<typename T> struct eval_texture_op
{
...
template<typename T> T eval_texture_op<T>::operator()(const ConstantTexture<T>& t) const
{
    return t.value;
}

template<typename T> T eval_texture_op<T>::operator()(const ImageTexture<T>& t) const
{
...
template<typename T> T eval_texture_op<T>::operator()(const CheckerboardTexture<T>& t) const
{
    Vector2 local_uv{modulo(uv[0] * t.uscale + t.uoffset, Real(1)), modulo(uv[1] * t.vscale + t.voffset, Real(1))};
...
inline ImageTexture<Spectrum> make_image_spectrum_texture(const std::string& texture_name,
...
inline ImageTexture<Real> make_image_float_texture(const std::string& texture_name,
                                                   const fs::path& filename,
                                                   TexturePool& pool,
                                                   Real uscale  = 1,
                                                   Real vscale  = 1,
...
inline CheckerboardTexture<Spectrum> make_checkerboard_spectrum_texture(const Spectrum& color0,
